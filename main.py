...
def p_mapping(pnode, i, e, A):
    ...
    return children_breakpoint + children_dist + jumping_penalty
...
def q_mapping(qnode, i, e, A):
    ...
    all_children_flipped = True
    for child in qnode.children:    # checking if all children were flipped
        if not child.flipped:
            all_children_flipped = False

    if all_children_flipped:
        qnode.direction = (-1)*qnode.direction

    # if qnode.flipped and not all_children_flipped or all_children_flipped and not qnode.flipped:
    #     children_dist = children_dist + 3   # penalty for changing order of q-node

    qnode.l = i
    qnode.r = e

    children_breakpoint = 3*get_bp_from_children_orders(children_order_tree, children_order_string)

    return children_dist + children_breakpoint
...
def get_pqtrees_dict_with_outliers(path):
    outlier_index = find_outliers(path)
...
def run_MEM4(path):
    ...
    MEM2 = add_penalty_for_outlier(MEM2, outlier_index, 3)

    return MEM2


def get_all_MEM4_dicts():
    print("\n\n---------- calculating MEM4 ---------------\n")
    csbs_files_paths = glob.glob(r'all_examples\*.txt')
...
def change_keys(MEM4_by_file_dict):
    ...
    for path in MEM4_by_file_dict.keys():
        new_dict[path] = {}
        csbs_names_dict = get_csbs_names_dict("all_examples/" + path + ".txt")
